...
export class FormModalAPComponentProveedor {
  @Input() public modif=false; 
  @Input() public clientem: ProveedorModel;
  myForm: FormGroup;
  filePath;
  pelicula: ProveedorModel
  ...
  ngOnInit(): void {
    ...
    this.emailm.setValue(this.clientem.email, {
      onlySelf: true
    })
  }
  }
...
if(this.myForm.valid){
  if(!this.modif){
this.service.crearProveedor(formValue).subscribe(resp=>{
  this.isSubmitted=false;
  this.activeModal.close();
}
  )
}else{
  this.service.actualizarProveedor(formValue).subscribe(resp=>{
    this.isSubmitted=false;
    this.activeModal.close();
  }
    )
